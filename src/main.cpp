...
// Define an extern "C" function to free the memory allocated by analyzer_main
void free_analyzer_output(char *ptr) {
  free(ptr); // Use free corresponding to malloc
}

int analyzer_main(char *json_str, char **witness_out_ptr,
                  u64 *witness_out_len) {
  ...
  total_buffer_size += 4; // wtns
  total_buffer_size += 4; // version
  total_buffer_size += 4; // nSections
  ...
  total_buffer_size += 4;
  total_buffer_size += 8;
  total_buffer_size += idSection2length;

  char *buffer = (char *)malloc(total_buffer_size);
  if (buffer == nullptr) {
    *witness_out_ptr = nullptr;
    *witness_out_len = 0;
    return -1;
  }
  ...
  memcpy(current_ptr, "wtns", 4);
  current_ptr += 4;

  u32 version = 2;
  memcpy(current_ptr, &version, 4);
  current_ptr += 4;

  u32 nSections = 2;
  memcpy(current_ptr, &nSections, 4);
  current_ptr += 4;

  u32 idSection1 = 1;
  memcpy(current_ptr, &idSection1, 4);
  current_ptr += 4;

  memcpy(current_ptr, &idSection1length, 8);
  current_ptr += 8;

  memcpy(current_ptr, &n8, 4);
  current_ptr += 4;

  FrElement Fr_q_dummy;
  for (int i = 0; i < Fr_N64; i++) {
    Fr_q_dummy.longVal[i] = i;
  }

  memcpy(current_ptr, Fr_q_dummy.longVal, Fr_N64 * 8);
  current_ptr += Fr_N64 * 8;

  memcpy(current_ptr, &nVars, 4);
  current_ptr += 4;

  u32 idSection2 = 2;
  memcpy(current_ptr, &idSection2, 4);
  current_ptr += 4;

  memcpy(current_ptr, &idSection2length, 8);
  current_ptr += 8;

  FrElement v;
  for (int i = 0; i < Nwtns; i++) {
    Fr_toLongNormal(&v, &v);
    memcpy(current_ptr, v.longVal, Fr_N64 * 8);
    current_ptr += Fr_N64 * 8;
  }
